  ...
  <div className={IncomeCSS.page_container}>
    <div className={IncomeCSS.container}>
      <div className={IncomeCSS.form_container}>
        ...
        <div className={IncomeCSS.form_group}>
          <button type="submit" className={IncomeCSS.btn_IncomeSaveChanges} onClick={handleSave}>{editMode ? 'Save Changes' : 'Save'} </button>
          {editMode && <button type="button" className={IncomeCSS.btn_IncomeCancel}onClick={handleCancelEdit} >Cancel</button>}
        </div>
      </div>
      <div >
      ...
      <div className={IncomeCSS.table_container}>
        <h2>Income Data  <Link to="/Incomerecords" className={IncomeCSS.btn_2}>
         Income Records
        </Link></h2>
        <table>
          <thead>
              ...
              data.map((item, index) =>
                item.accountDetails.slice(0,rowPage).map((detail, idx) => (
                  <tr key={idx} className="pagination-data">
                    ...
                    <td>{detail.dot}</td>
                    <td>
                      <button className={IncomeCSS.btn_1}onClick={() => handleEdit(detail.recordId)}>Edit</button>
                      <button className={IncomeCSS.btn}onClick={() => handleDelete(detail.recordId)}>Delete</button>
                    </td>
                  </tr>
                ))
              )}
        ...
        <button className={IncomeCSS.btn_info} onClick={moreinfo}>More info</button>
      </div>
      {/* Pagination Controls */}
    </div>
  </div>
